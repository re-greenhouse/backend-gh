...
export class EmployeeCreatedListener {
  private static readonly eventName: string = 'employee.created';
  ...
  @OnEvent(EmployeeCreatedListener.eventName)
  async handleOrderCreatedEvent(
    ...
    const template = await this.findTemplateRepository.findByEventName(
      EmployeeCreatedListener.eventName,
    );

    if (!template) {
      throw new GrpcNotFoundException(
        `Template for event '${EmployeeCreatedListener.eventName}' not found'`,
      );
    }
